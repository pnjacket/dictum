...
Use the **`install-dictum`** skill (run it from this Dictum checkout, targeting the product repo). It copies the five skills + five agents into the target's `.claude/`, vendors (or references) the standard material, and writes the `CLAUDE.md` path pointer so bare references to `STANDARD.md` / `concerns/11.x` / `templates/` resolve. The equivalent manual steps:
